...
def crossval(args, dm):
    ...
    for train_index, test_index in kf.split(X):
        ...
        probs_ = trainer.predict(system, df_test)
...
def inspect(all_df):
    ...
    ranked_label_issues = find_label_issues(
        all_df["label"], prob, return_indices_ranked_by='self_confidence')
...
def retrain_retest(args, all_df):
    ...
    # Calculate the indices for each dataset split based on their original
    # sizes
    ...
    ...

    # Update the dataframes in the datasets
    dm.train_dataset.data = all_df.iloc[range(train_size)]
    dm.dev_dataset.data = all_df.iloc[range(train_size, 
                                            train_size + dev_size)]
    dm.test_dataset.data = all_df.iloc[range(train_size + dev_size, 
                                             len(all_df))]
